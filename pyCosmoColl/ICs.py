...
def IsoSphere(Ndark, Ngas,rsphere):
	...
	while Nglass < 4*max(Ndark,Ngas):
	    ...
	    bs *= 2
	    Nglass *= 8
	print "mirroring complete. Number of mirrors:", bs, Nglass, Ndark, Ngas
...
zstart  = starting redshift of the simulation. Suggestion: use 4
...
#	print "fracH: ", fracH, "\n"
	...
	if Ndark > 0:           #Set the Hubble flow for each particle 
	        velDark = np.zeros_like(posDark)
	        vInFall_Dark = (2.0/3.0)*delta(delta0,rDark,rsphere)*rDark*H
	        vHubDark =  rDark_Perturb*H
	if Ngas > 0:
	        velGas = np.zeros_like(posGas)
	        vInFall_Gas = (2.0/3.0)*delta(delta0,rGas,rsphere)*rGas*H
	        vHubGas = rGas_Perturb*H
	if Ndark > 0:
		if np.size(np.where(vHubDark < vInFall_Dark)) > 0: print "uh oh, some DM particles will start with inward velocities!"
	if Ngas > 0:
		if np.size(np.where(vHubGas < vInFall_Gas)) > 0: print "uh oh, some GAS particles will start with inward velocities!"
	...
	if Lambda > 0:
		...
		E = np.abs(Eg + Ek)
	        ...
	        if Ndark > 0:
	                ...
	                vphi_Dark = rxyDark * w
	                if np.size(np.where(vphi_Dark**2 > 0.1*(vHubDark - vInFall_Dark)**2))>0: print "uh oh! bulk motion too big??"
	...
	if TdivR > 0 and Ndark > 0:
		...
		vInFall_Dark  = np.sqrt(vInFall_Dark**2 - vmag**2)
	        ...
	        vTanRand = np.zeros_like(posDark)
	        vTanRand[:,0] = vmag*(np.sin(Omega)*np.cos(Theta)*np.cos(Phi) - np.cos(Omega)*np.sin(Phi))
	        vTanRand[:,1] = vmag*(np.cos(Omega)*np.cos(Phi) + np.sin(Omega)*np.cos(Theta)*np.sin(Phi))
...
#	if Ngas > 0: vTanTotGas = np.sqrt(velGas[:,0]**2+velGas[:,1]**2+velGas[:,2]**2)
	...
	if OverSamp <= 0:
	        print "WARNING: I am assuming you mean Oversampled is 1. Must be > 0..."
	        OverSamp = 1
	gsoft = (MDark[0]/MpartRef)**(1./3.)*OverSamp*softRef
	if gsoft  > partSep:
	        print "WARNING: original grav. softening", gsoft, " is too big! using ", partSep, " instead"
	        gsoft = partSep
